...
router.get("/selectall", (req, res)=>{
    dbconn.query('select * from image ', (err, result, fields)=>{
        res.json(result);
    });
});
...
router.get("/select/:id", (req, res) => {
    ...
    dbconn.query("SELECT * FROM image WHERE imid = ?" , [id], (err, result, fields) => {
    if (err) throw err;
      res.json(result);
    });
});
...
router.post("/insertImg", (req, res) => {
    ...
    if (req.query){
        ...
        let sql = `
            INSERT INTO image (uid, name, score, voteTOTAL, url)
            SELECT ?, ?, ?, ?, ?
            FROM DUAL
            WHERE NOT EXISTS (
                SELECT 1
